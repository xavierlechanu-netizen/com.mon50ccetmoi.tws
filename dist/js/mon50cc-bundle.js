...
window.LootSystem = {
    ...
    listenToLootDrops: function() {
        ...
        firebase.firestore().collection("loot_drops")
            .where("expiresAt", ">", now)
    ...
    checkDistance: function() {
        ...
        const pos = new google.maps.LatLng(window.currentPosition.lat, window.currentPosition.lng);
        
        for (const [lootId, marker] of Object.entries(this.lootMarkers)) {
            const lootPos = marker.getPosition();
            const dist = google.maps.geometry.spherical.computeDistanceBetween(pos, lootPos);
    ...
    claimLoot: async function(lootId) {
        ...
        try {
            ...
            console.log("[LootSystem] Claim error:", e);
        }
    },
    ...
    devSpawnLoot: function() {
        ...
        firebase.firestore().collection("loot_drops").add({
        ...
        console.log("Faux loot spawn.");
    }
};

document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => { window.LootSystem.init(); }, 4000);
});
...
window.CortegeSystem = {
            ...
            .collection("members").onSnapshot(snap => {
                snap.docChanges().forEach(change => {
                    ...
                    if (change.type === "added" || change.type === "modified") {
                        ...
                        if (data.lastSignal && (!oldData || !oldData.lastSignal || oldData.lastSignal.time !== data.lastSignal.time)) {
    ...
    sendSignal: async function(icon, text) {
        ...
        try {
            await firebase.firestore().collection("cortege_sessions").doc(this.sessionId)
                .collection("members").doc(window.session.uid).update({
                    lastSignal: {
                        icon: icon,
                        text: text,
                        time: Date.now()
    ...
    handleNewSignal: function(senderName, signal) {
        ...
        toast.style.cssText = "position:fixed;top:80px;left:50%;transform:translateX(-50%);background:rgba(0,255,204,0.9);color:#000;padding:15px 25px;border-radius:25px;z-index:99999;font-weight:bold;font-family:'Outfit', sans-serif;box-shadow:0 0 20px rgba(0,255,204,0.5);font-size:1.1rem;opacity:0;transition:opacity 0.3s;display:flex;align-items:center;gap:10px;";
    ...
    checkWeatherForRoute: async function(lat, lng) {
        try {
            ...
            if (data && data.current_weather) {
                ...
                if (windspeed > 40) { // Vent > 40 km/h (dangereux en 50cc léger)
...
window.GarageMarket = {
    ...
    listenToTrades: function() {
        ...
        firebase.firestore().collection("garage_trades")
            .where("isActive", "==", true)
            .onSnapshot(snapshot => {
                snapshot.docChanges().forEach(change => {
                    const data = change.doc.data();
...
window.RoadbookSystem = {
    ...
    init: function() {
        if (!window.session || !window.session.uid) return;
        console.log("[RoadbookSystem] Initializing...");
    ...
    toggleRecording: function() {
        ...
        if (this.isRecording) {
            ...
            if (this.currentPath.length > 5) {
                const title = prompt("Enregistrement terminé. Entrez un nom pour votre Roadbook (ex: Balade en Vallée de Chevreuse) :");
                if (title) {
            ...
            if (typeof speak === 'function') speak("Enregistrement du tracé activé.");
            
            this.recordInterval = setInterval(() => {
                if (window.currentPosition) {
                    this.currentPath.push({lat: window.currentPosition.lat, lng: window.currentPosition.lng});
                }
    ...
    listenToRoadbooks: function() {
        ...
        firebase.firestore().collection("roadbooks")
...
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => { window.RoadbookSystem.init(); }, 4000);
});
...
window.PitStopSystem = {
    ...
    init: function() {
        if (!window.session || !window.session.uid) return;
        console.log("[PitStopSystem] Initializing...");
    ...
    listenToPitStops: function() {
        ...
        firebase.firestore().collection("pit_stops")
            .onSnapshot(snapshot => {
                snapshot.docChanges().forEach(change => {
                    const data = change.doc.data();
                    const id = change.doc.id;
    ...
    simulateOBDFuelCheck: function() {
        ...
        if (window.isRiding || window.forceOBDCheck) {
            ...
            if (window.obdFuelLevel <= 15 && !this.isFuelLow) {
                this.isFuelLow = true;
                if (typeof speak === 'function') {
                    speak("Alerte O B D. Niveau de carburant critique, inférieur à 15 pourcents. Voulez-vous que je vous guide vers la station la plus proche ?");
                }
                ...
                if (window.currentPosition && typeof google !== 'undefined') {
                    ...
                    for (const [id, marker] of Object.entries(this.markers)) {
                        const dist = google.maps.geometry.spherical.computeDistanceBetween(myPos, marker.getPosition());
                        if (dist < minDist) {
                            minDist = dist;
                            closestGas = marker;
                        }
...
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => { window.PitStopSystem.init(); }, 4000);
});
...
window.PrivacyManager = {
    ...
    init: function() {
        console.log("[PrivacyManager] Initializing RGPD compliance...");
    ...
    showConsentBanner: function() {
        ...
        if (!banner) {
            ...
            banner.style.cssText = "position:fixed; bottom:0; left:0; width:100%; background:rgba(0,0,0,0.95); border-top:2px solid #00ffcc; z-index:999999; padding:20px; display:flex; flex-direction:column; align-items:center; justify-content:center; text-align:center; color:white; backdrop-filter:blur(10px); box-shadow:0 -5px 20px rgba(0,255,204,0.3);";
    ...
    setConsent: function(agreed) {
        ...
        if (!agreed) {
            alert("Vous avez refusé. L'application fonctionnera en mode restreint. Vous n'apparaîtrez pas sur la carte des autres pilotes.");
            this.toggleGhostMode(true); // Force invisible
        } else {
            this.toggleGhostMode(false);
            if (typeof speak === 'function') speak("Paramètres de confidentialité enregistrés. Bienvenue dans la communauté.");
        }
    },
    
    toggleGhostMode: function(forceState = null) {
        if (!this.consentGiven && forceState === false) {
            alert("Vous devez d'abord accepter le partage GPS pour désactiver le mode fantôme.");
            this.showConsentBanner();
            return;
        }
        
        this.ghostModeActive = forceState !== null ? forceState : !this.ghostModeActive;
        ...
        if (window.session && window.session.uid && typeof firebase !== 'undefined') {
            firebase.firestore().collection("users").doc(window.session.uid).update({
        ...
        if (forceState === null && typeof speak === 'function') {
            speak(this.ghostModeActive ? "Mode fantôme activé. Vous êtes invisible." : "Mode fantôme désactivé. Vous êtes visible.");
        }
    },
    ...
    showSettingsModal: function() {
        ...
        if (!modal) {
            ...
            modal.style.cssText = "position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.9);z-index:99999;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(5px);";
            document.body.appendChild(modal);
        }
    ...
    deleteMyData: async function() {
        ...
        if (typeof firebase === 'undefined' || !window.session || !window.session.uid) {
        ...
        try {
            ...
            console.log("Suppression des données pour UID:", uid);
            ...
            alert("✅ Vos données ont été supprimées avec succès (Droit à l'oubli). Vous allez être déconnecté.");
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => { window.PrivacyManager.init(); }, 1500);
});
...
function runCinematicStartup() {
    ...
    // Needle Sweep 0 -> 80 -> 0
    setTimeout(() => {
        if(needle) needle.style.transform = 'rotate(40deg)'; // 120 -> 40 pour Ãªtre proportionnel
...
document.addEventListener('DOMContentLoaded', () => {
    console.log("DOM PrÃªt. En attente du SDK Maps...");
});
...
window.showPage = function(page) {
    ...
    if(page === 'stats') {
        if(typeof content !== 'undefined') content.innerHTML = `<h3><i class="fa-solid fa-chart-line"></i> ${t('stats_title')}</h3>
        ...
        if(typeof content !== 'undefined') content.innerHTML = `<h3><i class="fa-solid fa-trophy"></i> Saisons de Pilote</h3>
        ...
        if(typeof content !== 'undefined') content.innerHTML = `<h3><i class="fa-solid fa-map-location-dot"></i> Roadbooks Communautaires</h3>
        ...
        if(typeof content !== 'undefined') content.innerHTML = `<h3><i class="fa-solid fa-warehouse"></i> ${t('garage_title')}</h3>
            ...
            <div class="card" style="border: 1px solid var(--neon-blue); background: rgba(0, 210, 255, 0.05);">
                ...
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; text-align:center;">
                    <div style="background:rgba(0,0,0,0.3); padding:10px; border-radius:10px;">
                        ...
                        <strong style="font-size:1.2rem; color:var(--accent);">${maxLeanAngle}Â°</strong>
                    </div>
                    <div style="background:rgba(0,0,0,0.3); padding:10px; border-radius:10px;">
                ...
                <button onclick="resetTelemetry()" style="width:100%; height:25px; margin-top:10px; background:transparent; border:1px solid #444; color:#666; font-size:0.6rem; border-radius:15px;">RÃ‰INITIALISER LES STATS</button>
            </div>
            ...
            <div id="maint-history" style="font-size:0.8rem; margin-top:15px; max-height:200px; overflow-y:auto;">
                ${history.length ? history.reverse().map(h => `<div style="padding:10px; background:rgba(255,255,255,0.05); margin-bottom:5px; border-radius:8px; border-left:3px solid ${h.certified ? '#2ecc71' : '#444'};">
                    ...
                    ${h.certified ? `<div style="font-size:0.6rem; color:#2ecc71; margin-top:5px;"><i class="fa-solid fa-certificate"></i> CERTIFIÃ‰ PAR : ${h.garage}</div>` : ''}
                </div>`).join('') : '<p style="color:#444; text-align:center;">Votre carnet est vide.</p>'}
            </div>`;
        renderDynamicGarage();
    } else if(page === 'group') {
        if(typeof content !== 'undefined') content.innerHTML = `<h3>Balade en Groupe</h3>
        ...
        if(typeof content !== 'undefined') content.innerHTML = `<h3>ItinÃ©raires Rodage</h3>
            <p>Routes limitÃ©es Ã  45 km/h pour prÃ©server votre moteur.</p>
            <button class="btn-insurance" onclick="startRodage('Paris-Boucle')">Boucle Zen (Paris)</button>
            <button class="btn-insurance" onclick="startRodage('Lyon-Quais')">Quais SaÃ´ne (Lyon)</button>`;
    } else if(page === 'insurance') {
        if(typeof content !== 'undefined') content.innerHTML = `<div class="card-insurance">
            ...
            <div class="broker-contact">
                ...
                <span>SpÃ©cialiste du jeune conducteur 50cc</span>
            </div>
            <p>BÃ©nÃ©ficiez de -15% sur votre assurance scooter en tant que membre.</p>
        </div>`;
    } else if(page === 'roadbooks') {
        if(typeof content !== 'undefined') content.innerHTML = `<h3>Roadbooks</h3>
            <div style="display:flex; gap:10px; margin-bottom:15px;">
                <button onclick="renderRoadbooks('all')" class="btn-insurance" style="flex:1; padding:8px; font-size:0.75rem;">Mes CrÃ©ations</button>
        ...
        if(typeof content !== 'undefined') content.innerHTML = `<h3><i class="fa-solid fa-robot"></i> ${t('expert_meca_title')}</h3>
            <p style="font-size:0.8rem; color:#aaa;">DÃ©crivez le symptÃ´me (bruit, fumÃ©e, panne...)</p>
            <textarea id="meca-query" placeholder="Ex: Mon scoot broute Ã  l'accÃ©lÃ©ration..." style="width:100%; height:80px; margin-top:10px; background:#111; color:white; border:1px solid #ffb703; border-radius:8px; padding:10px;"></textarea>
        ...
        if (window.session && window.session.isGuest) {
            alert("AccÃ¨s rÃ©servÃ© aux membres inscrits ! ðŸ›µ");
            return;
        }
        if(typeof content !== 'undefined') content.innerHTML = `<h3><i class="fa-solid fa-scale-balanced"></i> ${t('arbitre_title')}</h3>
            <p style="font-size:0.8rem; color:#aaa; margin-bottom:15px;">Posez votre question sur la rÃ©glementation 50cc (dÃ©bridage, Ã©quipement, contrÃ´les...).</p>
            ...
            <div style="display:flex; gap:10px;">
                <input type="text" id="arbitre-query" placeholder="Ex: Mon pot est-il homologuÃ© ?" style="flex:1; background:#111; color:white; border:1px solid #444; border-radius:20px; padding:10px 15px; font-size:0.9rem;">
        ...
        if(typeof content !== 'undefined') content.innerHTML = `<div class="card-insurance" style="border: 2px solid #b700ff; background: rgba(20, 10, 40, 0.9);">
            <div class="insurance-badge" style="background: #b700ff; color: white;">IA PrÃ©dictive & Courtier</div>
            <h3 style="color: #00d2ff;"><i class="fa-solid fa-microchip"></i> IA PrÃ©dictive</h3>
            ...
            <div class="glassmorphism" style="padding:15px; margin-bottom:20px; background: rgba(0,0,0,0.4);">
                ...
                <ul style="color: #aaa; font-size: 0.8rem; text-align: left; padding-left: 20px;">
                    <li><strong style="color: #fff;">-20% de rÃ©duction</strong> sur votre assurance tous risques grÃ¢ce Ã  l'IA PrÃ©dictive.</li>
                    <li><strong style="color: #fff;">Garantie panne 0 km</strong> incluse avec dÃ©pannage express.</li>
                    <li><strong style="color: #fff;">Bonus de bonne conduite</strong> (Rouler & Gagner convertible en rÃ©ductions).</li>
                </ul>
            ...
            <button class="btn-insurance" style="width:100%; background: linear-gradient(135deg, #b700ff, #00d2ff); color: white; font-weight: bold; border: none; padding: 15px; border-radius: 10px;" onclick="PredictiveMeca.checkAlerts(); alert('L\'IA analyse vos donnÃ©es de tÃ©lÃ©mÃ©trie actuelles... Aucun risque de serrage moteur dÃ©tectÃ© pour le moment. Vous roulez de maniÃ¨re optimale !')"><i class="fa-solid fa-bolt"></i> LANCER L'ANALYSE IA</button>
        </div>`;
    } else if(page === 'profile') {
        if(typeof content !== 'undefined') content.innerHTML = `<h3><i class="fa-solid fa-user-pen"></i> Mon Profil</h3>
            <div class="glassmorphism" style="padding:20px; margin-bottom:20px;">
                ...
                <label style="color:#aaa; font-size:0.8rem;">ModÃ¨le de scooter :</label>
        ...
        if(typeof content !== 'undefined') content.innerHTML = `<h3><i class="fa-solid fa-building-shield"></i> Portail Expert Assurance</h3>
            <p style="font-size:0.8rem; color:#aaa; margin-bottom:20px;">AccÃ¨s sÃ©curisÃ© pour les compagnies d'assurance et experts judiciaires.</p>
        ...
        if(typeof content !== 'undefined') content.innerHTML = `<h3><i class="fa-solid fa-microscope"></i> Labo MÃ©ca : StÃ©thoscope IA</h3>
            <p style="font-size:0.8rem; color:#aaa; margin-bottom:20px;">Analyse biomÃ©trique de la santÃ© de votre moteur via les capteurs du smartphone.</p>
            ...
            <div class="glassmorphism" style="padding:20px; text-align:center;">
                ...
                <p style="font-size:0.7rem; color:#888; margin-top:10px;">Posez le tÃ©lÃ©phone sur la selle, moteur allumÃ© au ralenti.</p>
            </div>
            <div id="pulse-result"></div>`;
    } else if(page === 'ants_wallet') {
        const passport = Wallet.getSafetyPassport();
        if(typeof content !== 'undefined') content.innerHTML = `<h3><i class="fa-solid fa-building-columns"></i> Mon Coffre-Fort ANTS</h3>
            <p style="font-size:0.75rem; color:#aaa; margin-bottom:20px;">Titres sÃ©curisÃ©s et Passeport SÃ©curitÃ© certifiÃ© par mon50ccetmoi.</p>
            
            <div class="glassmorphism" style="padding:15px; margin-bottom:15px; border-left:4px solid #2ecc71;">
                <h4 style="font-size:0.9rem; color:#2ecc71;"><i class="fa-solid fa-id-card"></i> Passeport SÃ©curitÃ© Digital</h4>
                <div style="font-size:0.8rem; margin-top:5px; color:#ddd;">
                    ...
                    SantÃ© Moteur: <span style="color:#2ecc71;">${passport.engine_health}</span>
                </div>
            </div>

            <div class="menu-list" style="margin-top:20px;">
                <div id="ants-docs-container" style="margin-bottom:15px;"></div>
                <li onclick="window.uploadDocument('carte_grise')"><i class="fa-solid fa-camera"></i> NumÃ©riser Carte Grise</li>
                <li onclick="window.uploadDocument('permis_am')"><i class="fa-solid fa-address-card"></i> NumÃ©riser Permis AM</li>
            ...
            <p style="font-size:0.65rem; color:#666; text-align:center; margin-top:20px;">Note : Ce coffre-fort facilite les contrÃ´les mais ne remplace pas les documents originaux selon la lÃ©gislation en vigueur.</p>`;
    } else if(page === 'meca_lab') {
        if(typeof content !== 'undefined') content.innerHTML = `<h3><i class="fa-solid fa-oil-can"></i> Le Sorcier de la MÃ©ca</h3>
            <div class="glassmorphism" style="padding:20px; margin-bottom:20px;">
                <h4 style="color:var(--accent);">CALCULATEUR DE MÃ‰LANGE</h4>
                <div style="margin-top:15px;">
                    ...
                    <button onclick="const vol = MecaWizard.calculateMix(document.getElementById('mix-liters').value, document.getElementById('mix-percent').value); document.getElementById('mix-res').innerHTML = vol + ' ml d\'huile Ã  ajouter';" 
        ...
        if(typeof content !== 'undefined') content.innerHTML = `<h3><i class="fa-solid fa-circle-info"></i> Ã€ Propos</h3>
        ...
        const availableChallenges = [
            ...
            { name: "Le VÃ©lomoteur", goal: 50, unit: "km" }
        ];

        // Rotation tous les 14 jours basÃ©e sur l'Unix Time
        ...
        if(typeof content !== 'undefined') content.innerHTML = `<div class="card" style="border:1px solid #9b59b6;">
            <div style="display:flex; justify-content:space-between; align-items:center;">
            ...
            <div style="background:rgba(255,255,255,0.05); border-radius:10px; padding:15px; margin-top:15px;">
                ...
                <p style="font-size:0.8rem; color:#888; margin-top:10px; text-align:center;">ðŸŽ–ï¸ Vous avez rÃ©ussi <strong>${wins}/150</strong> dÃ©fis pour le Badge Pro</p>
            </div>
              ...
              ` : (progress >= 100 ? `<button class="btn-insurance" style="margin-top:10px; width:100%; background:#333; color:#aaa; cursor:not-allowed;" disabled>RÉCOMPENSE DE 5 BVC RÉCUPÉRÉE</button>` : '')}
        </div>`;
    } else if(page === 'roadbooks') {
        if(typeof content !== 'undefined') content.innerHTML = `<h3><i class="fa-solid fa-map-location-dot"></i> Navigation & Roadbooks</h3>
            <div class="glassmorphism" style="padding:20px; border-left:4px solid #f1c40f; margin-bottom:20px;">
                <h4 style="color:#f1c40f;"><i class="fa-solid fa-stopwatch"></i> CHRONOS GUARD (ZÃ©ro Retard)</h4>
                <p style="font-size:0.75rem; margin-top:5px; color:#aaa;">RÃ©glez votre heure d'arrivÃ©e cible. L'app inclut votre temps d'Ã©quipement (5 min).</p>
            ...
            <p style="text-align:center; padding:40px; color:#666;">Liste de vos roadbooks sauvegardÃ©s...</p>`;
    } else if(page === 'arbitre') {
        if (window.Blackbox && typeof window.Blackbox.showLitigationInfo === "function") {
            window.Blackbox.showLitigationInfo();
        } else {
            if(typeof content !== 'undefined') content.innerHTML = `<h3><i class="fa-solid fa-scale-balanced"></i> Arbitre de la Route</h3><p>Service Blackbox momentanÃ©ment indisponible.</p>`;
        }
    } else if(page === 'privacy') {
        if(typeof content !== 'undefined') content.innerHTML = `<h3>Mentions LÃ©gales & ConfidentialitÃ©</h3>
            <div style="font-size:0.8rem; line-height:1.4; color:#ccc;">
                <p><strong>Ã‰diteur :</strong> mon50ccetmoi (Engineering Unit)</p>
        ...
        if(typeof content !== 'undefined') content.innerHTML = `<h3><i class="fa-solid fa-lightbulb"></i> Conseils de Pro 50cc</h3>
            <p style="font-size:0.7rem; color:#aaa; margin-bottom:15px;">Fiches techniques rÃ©digÃ©es par nos experts et les garages certifiÃ©s.</p>
            ...
            <div id="pro-tips-container">
                <div class="card" style="border-left:4px solid #f39c12;">
                    ...
                    <p style="font-size:0.8rem; margin-top:5px;"><strong>Bougie :</strong> Une bougie propre (couleur chocolat) = un moteur qui dure. Si elle est noire, votre mÃ©lange est trop riche.</p>
                </div>
                ...
                <div class="card" style="border-left:4px solid #e74c3c;">
                    ...
                    <h4 style="color:#e74c3c;"><i class="fa-solid fa-scale-balanced"></i> Loi & SÃ©curitÃ©</h4>
                    <p style="font-size:0.8rem; margin-top:5px;"><strong>Bridage :</strong> Le dÃ©bridage est interdit sur voie publique. En cas d'accident, votre assurance peut refuser de payer.</p>
                </div>
            </div>`;
    } else if(page === 'pro-space') {
        const isCertified = window.session?.isCertifiedGarage || false;
        if(typeof content !== 'undefined') content.innerHTML = `<h3><i class="fa-solid fa-briefcase"></i> ${t('pro_space_title')}</h3>
            <div class="card" style="border:1px solid #3498db; background: rgba(52, 152, 219, 0.05);">
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    <strong>VisibilitÃ© Mobile</strong>
                ...
                <small style="font-size:0.6rem; color:#aaa; margin-top:5px; display:block;">Si activÃ©, vous apparaissez en bleu sur la carte des pilotes.</small>
            </div>

            <div class="card">
                <label style="font-size:0.8rem; display:block; margin-bottom:5px;">Statut immÃ©diat de l'atelier</label>
                <select id="garage-status-select" onchange="updateGarageStatus(this.value)" class="scooter-brand-select" style="width:100%; background:#111;">
                    <option value="dispo" selected>âœ… Prise en charge immÃ©diate</option>
                    <option value="busy">â³ RDV nÃ©cessaire (>48h)</option>
            ...
            <div class="card" style="border:1px solid #f1c40f;">
                ...
                <button onclick="publishFlashOffer()" class="btn-insurance" style="background:#f1c40f; color:black; margin-top:10px; width:100%; font-size:0.8rem;">Diffuser Ã  la communautÃ©</button>
            </div>
            ...
            <div class="card" style="text-align:center; background:rgba(52, 152, 219, 0.05); border:1px solid #3498db;">
                ...
                <h4 style="margin:10px 0; color:#fff;">Droit d'EntrÃ©e & Certification</h4>
                <p style="font-size:0.7rem; color:#aaa; margin-bottom:10px;">Devenez <strong>Garage CertifiÃ©</strong> pour seulement <strong>50â‚¬ TTC</strong> (Paiement unique).</p>
                <ul style="font-size:0.65rem; color:#ccc; list-style:none; padding:0; text-align:left; margin-bottom:15px;">
                    <li>âœ… Badge <strong>CertifiÃ© mon50ccetmoi</strong></li>
                    <li>ðŸš€ <strong>Boost de visibilitÃ©</strong> sur la carte</li>
                    <li>ðŸ› ï¸ AccÃ¨s illimitÃ© aux fiches techniques</li>
                    <li>ðŸ‘” PrioritÃ© dans les rÃ©sultats de recherche</li>
                </ul>
                <button onclick="payGarageEntryFee()" class="btn-insurance" style="background:#f1c40f; color:black; font-weight:bold;">S'acquitter du droit d'entrÃ©e (50â‚¬)</button>
                
                <div style="margin-top:15px; padding-top:15px; border-top:1px solid #444;">
                    <p style="font-size:0.7rem; color:#2ecc71;"><strong>ðŸŽ OPTION "CROISSANCE" GRATUITE :</strong></p>
                    <p style="font-size:0.6rem; color:#aaa;">Offrez <strong>-10% de rÃ©duction</strong> aux membres sur prÃ©sentation de l'app et soyez <strong>exonÃ©rÃ©</strong> des 50â‚¬ !</p>
                    <button onclick="applyPartnerExemption()" class="btn-insurance fa-beat" style="background:transparent; border:1px solid #2ecc71; color:#2ecc71; margin-top:5px; font-size:0.8rem; font-weight:bold;">REJOINDRE LE RÃ‰SEAU GRATUITEMENT (-10%)</button>
                </div>
            </div>` : `
            <div class="card" style="text-align:center; background:rgba(46, 204, 113, 0.1); border:1px solid #2ecc71;">
                ...
                <p style="font-size:0.8rem; color:#2ecc71; margin-top:5px;"><strong>Statut PRO CertifiÃ© Actif</strong></p>
                <small style="font-size:0.6rem; color:#aaa;">Votre visibilitÃ© est boostÃ©e au maximum.</small>
            </div>`}
        ...
        if(typeof content !== 'undefined') content.innerHTML = `<h3><i class="fa-solid fa-heart"></i> ${t('donate_title')}</h3>
            <div class="card" style="text-align:center; background: linear-gradient(135deg, rgba(233, 30, 99, 0.1), rgba(0,0,0,0)); border: 1px solid #e91e63;">
                ...
                <p style="font-size:0.9rem; line-height:1.5;"><strong>mon50ccetmoi</strong> est un projet de passionnÃ©, dÃ©veloppÃ© sur mon temps libre pour la communautÃ© des pilotes de 50cc.</p>
                <p style="font-size:0.8rem; color:#aaa; margin-top:10px;">L'application restera 100% gratuite, mais les dons aident Ã  payer les serveurs (Google Maps API, Firebase) et Ã  financer les futures mises Ã  jour.</p>
                
                <div style="margin-top:20px; display:flex; flex-direction:column; gap:10px;">
                    <a href="https://www.buymeacoffee.com/mon50cc" target="_blank" class="btn-insurance" style="background:#ffdd00; color:black; text-decoration:none;">â˜• Offrir un cafÃ© (Badge MÃ©cÃ¨ne ðŸ’–)</a>
                ...
                <p style="font-size:0.7rem; color:#666; margin-top:15px;">ðŸŽ Chaque don dÃ©bloque le badge exclusif **"MÃ©cÃ¨ne"** sur votre profil et sur la carte communautaire !</p>
            </div>
        `;
        ...
        if(typeof content !== 'undefined') content.innerHTML = `<h3><i class="fa-solid fa-shield-heart"></i> ${t('security_title')}</h3>
            ...
            <div class="card" style="display:flex; justify-content:space-between; align-items:center;">
                <div>
                    ...
                    <small style="font-size:0.6rem; color:#aaa;">Alerte si arrÃªt prolongÃ© suspect</small>
                </div>
            ...
            <div class="card" style="background:rgba(255,255,255,0.05); text-align:center;">
                ...
                <strong style="font-size:0.8rem;">DÃ©tecteur G-Force : ACTIF</strong><br>
                <small style="font-size:0.6rem; color:#666;">Impact calibrÃ© Ã  4.5G</small>
            </div>`;
    }
    toggleMenu();
}
...
window.shareApp = async function() {
    const shareData = {
        ...
        "lang": "fr-FR",
        title: 'mon50ccetmoi',
        text: 'Rejoins la communautÃ© des scooters 50cc ! Navigation GPS, radars et sÃ©curitÃ©.',
    ...
    try {
        if (navigator.share) {
            ...
            alert("Lien copiÃ© ! Partage-le avec tes potes : " + window.location.origin);
        }
...
window.submitMecaV3 = function() {
    ...
    setTimeout(() => {
        res.innerHTML = `<div style="background:rgba(255,183,3,0.1); padding:15px; border-radius:10px; border-left:4px solid #ffb703;">
            ...
            Il est probable que votre bougie soit encrassÃ©e ou que le gicleur de votre carburateur soit bouchÃ©. 
            VÃ©rifiez l'Ã©tincelle et nettoyez votre cuve.
        </div>`;
    }, 2000);
}
...
window.addEventListener('devicemotion', (e) => {
    ...
    if(force > 45) { // Seuil d'impact (G-force importante)
        ...
        if (window.isGuardianActive && typeof triggerEmergencySOS === "function") {
            triggerEmergencySOS("Chute brutale dÃ©tectÃ©e par l'accÃ©lÃ©romÃ¨tre.");
        }
    }
});
...
window.JarvisEngine = {
    ...
    speak: function(text) {
        if (typeof window.speak === 'function') {
            ...
            console.log("[J.A.R.V.I.S 4.0] (Voix off) :", text);
        }
    },
    ...
    processQuery: function(transcript) {
        ...
        // Appel d'Urgence / SOS
        if (this.matchAny(t, ['urgence', 'accident grave', 'secours', 'police', 'samu', 'pompier', 'pompiers', 'aide-moi'])) {
            return { action: 'EMERGENCY_CALL', reply: `Attention. Mode urgence activé. Je prépare l'appel aux services de secours.` };
        }
        // Comparaison Carburant
        else if (this.matchAny(t, ['essence', 'carburant', 'plein', 'station', 'sec']) && this.matchAny(t, ['moins cher', 'prix', 'compare', 'où'])) {
        ...
        // Navigation Spécifique
        else if (this.matchAny(t, ['emmène-moi', 'itinéraire vers', 'aller à', 'guidage vers'])) {
            ...
            return { action: 'NAVIGATE_TO', payload: destination, reply: `Calcul de l'itinéraire optimal vers ${destination}.` };
        }
        // Maison
        ...
        else if (this.matchAny(t, ['animal', 'animaux', 'biche', 'sanglier', 'chien'])) {
            return { action: 'REPORT_ANIMAL', reply: `Présence animale confirmée et partagée. Soyez prudent.` };
        }
        else if (this.matchAny(t, ['radar', 'flics', 'contrôle', 'police'])) {
            return { action: 'REPORT_POLICE', reply: `Zone de contrôle signalée sur le radar communautaire.` };
        }
        // Social
        else if (this.matchAny(t, ['meute', 'amis', 'social', 'radar social', 'pilotes', 'motards'])) {
            return { action: 'SOCIAL_RADAR', reply: `Activation du balayage social. Recherche de pilotes alliés dans le secteur.` };
        }
        // Modes de conduite
        else if (this.matchAny(t, ['sensation', 'virage', 'sport', 'attaque', 'balade'])) {
            return { action: 'SENSATION_MODE', reply: `Mode sensation engagé. Optimisation de l'itinéraire pour le plaisir de conduite.` };
        }
        // Diagnostic Moto
        else if (this.matchAny(t, ['diagnostic', 'état', 'santé', 'mécanique', 'panne', 'moteur'])) {
            return { action: 'AI_DIAGNOSTIC', reply: `J'ouvre le panneau de télémétrie prédictive de votre engin.` };
        }
        // Météo
        else if (this.matchAny(t, ['météo', 'temps', 'pluie', 'pleuvoir', 'froid', 'chaud'])) {
            return { action: 'WEATHER_CHECK', reply: `Je vérifie les conditions météorologiques sur votre parcours actuel.` };
        }
        // Profil
        else if (this.matchAny(t, ['mon score', 'mon profil', 'mes points', 'mes statistiques'])) {
            return { action: 'OPEN_PROFILE', reply: `Affichage de vos statistiques et de votre profil de pilote.` };
        }
        // Statistiques de l'application
        else if (this.matchAny(t, ['combien d\'utilisateurs', 'statistiques', 'téléchargements', 'audience', 'pays'])) {
            return { action: 'APP_STATS', reply: `D'après mes dernières analyses en date du 4 juillet 2026, l'application compte 4 installations uniques. 3 pilotes sont en France, et nous avons 1 pilote en Indonésie.` };
        }
        // Identité / Blague
        else if (this.matchAny(t, ['blague', 'humour', 'fais-moi rire'])) {
            return { action: 'JOKE', reply: this.getRandomResponse('jokes') };
        }
        else if (this.matchAny(t, ['qui es-tu', 'ton nom', "t'appelles", 'que sais-tu faire'])) {
            return { action: 'IDENTITY', reply: `Je suis Jarvis, l'intelligence artificielle de Mon 50cc et Moi. Je suis connecté à votre télémétrie, au réseau communautaire et prêt à vous assister sur la route.` };
        }
        else if (this.matchAny(t, ['drogue', 'stupéfiant', 'stupéfiants', 'positif', 'fumé', 'joint', 'cannabis', 'thc', 'dépistage', 'test'])) {
    ...
    executeAction: function(result) {
        ...
        switch (result.action) {
            case 'EMERGENCY_CALL':
                ...
                else alert("⚠️ URGENCE : Appeler le 112");
                break;
            case 'COMPARE_GAS_PRICES':
                ...
                break;
            case 'NAVIGATE_TO':
                if (document.getElementById('route-search')) document.getElementById('route-search').value = result.payload;
                if (typeof window.searchDestination === 'function') window.searchDestination();
                break;
            case 'GO_HOME':
                if (document.getElementById('route-search')) document.getElementById('route-search').value = "Domicile";
                if (typeof window.searchDestination === 'function') window.searchDestination();
                break;
            case 'REPORT_HAZARD':
                if (typeof window.reportHazard === 'function') window.reportHazard('danger');
                break;
            case 'REPORT_ANIMAL':
                if (typeof window.reportHazard === 'function') window.reportHazard('animal');
                break;
            case 'REPORT_POLICE':
                if (typeof window.reportHazard === 'function') window.reportHazard('police');
                break;
            case 'SOCIAL_RADAR':
            ...
            case 'AI_DIAGNOSTIC':
                ...
                break;
            case 'WEATHER_CHECK':
                if (typeof window.showWeatherWidget === 'function') window.showWeatherWidget();
                break;
            case 'OPEN_PROFILE':
...
window.initVoiceAI = function() {
    ...
    window.voiceAI.onresult = function(event) {
        ...
        const jarvisFeedback = document.getElementById('jarvis-feedback-text');
        if (jarvisFeedback && !transcript.includes('oracle') && !transcript.includes('système') && !transcript.includes('jarvis')) {
...
window.CommunityGas = {
    ...
    compareAndShow: function() {
        if (typeof speak === 'function') speak("Connexion au flux Open Data du gouvernement et récupération des prix en temps réel.");
        
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                (position) => this.fetchGovData(position.coords.latitude, position.coords.longitude),
                (error) => {
    ...
    fetchGovData: async function(lat, lon) {
        try {
            ...
            console.error(e);
            if (typeof speak === 'function') speak("Impossible de contacter le serveur gouvernemental. Veuillez réessayer plus tard.");
        }
    },
    ...
    processGovData: function(records, myLat, myLon) {
        ...
        if (!records || records.length === 0) {
            if (typeof speak === 'function') speak("Je ne trouve aucune station service répertoriée dans un rayon de 3 kilomètres.");
            return;
        }
        ...
        records.forEach(record => {
            ...
            const dist = this.calculateDistance(myLat, myLon, record.geom.lat, record.geom.lon);
            ...
            } catch(e) { console.warn("Erreur parsing prix pour station", record.id); }
            ...
            if (sp98 || e10) {
                this.stations.push({
                    id: record.id,
                    name: record.adresse ? record.adresse : (record.ville || 'Station Inconnue'),
                    distance: dist,
                    brand: record.ville || 'Local',
                    prices: {
                        'SP98': sp98 ? { price: sp98, updatedAt: record.maj || new Date().toISOString(), updatedBy: 'Data Gouv' } : null,
                        'E10': e10 ? { price: e10, updatedAt: record.maj || new Date().toISOString(), updatedBy: 'Data Gouv' } : null
                    }
                });
            }
        });
    ...
    renderHUD: function(nearbyStations) {
        ...
        nearbyStations.forEach(station => {
            ...
            const price = hasSP98 ? station.prices['SP98'].price.toFixed(3) : (station.prices['E10'] ? station.prices['E10'].price.toFixed(3) : '--');
    ...
    openUpdateModal: function(stationId) {
        ...
        const hasSP98 = !!station.prices['SP98'];
        const fuelName = hasSP98 ? 'SP98' : 'E10';
        const currentPrice = hasSP98 ? station.prices['SP98'].price : (station.prices['E10'] ? station.prices['E10'].price : '1.800');

        const newPrice = prompt(`Prix officiel Gouv.fr : ${currentPrice}€.\nEntrez le nouveau prix constaté sur place pour le ${fuelName} :`, currentPrice);
    ...
    updateStationPrice: function(stationId, price, fuelType) {
        ...
        if (station) {
            ...
            alert(`Merci ! Le prix a été mis à jour à ${price}€ et synchronisé avec la communauté.\nVous gagnez +5 Points BVC.`);
...
class OBDManager {
    ...
    async connect() {
        try {
            console.log("Demande d'accès au périphérique Bluetooth OBD...");
            ...
            console.log('Appareil sélectionné :', this.device.name);
            ...
            this.txCharacteristic = await service.getCharacteristic(this.characteristicUuid);
            this.rxCharacteristic = this.txCharacteristic;

            await this.rxCharacteristic.startNotifications();
            this.rxCharacteristic.addEventListener('characteristicvaluechanged', (e) => this.handleData(e));
    ...
    onDisconnected() {
        console.log('Périphérique OBD déconnecté.');
        this.isConnected = false;
        this.stopPolling();
        this.dispatchStateChange(false);
    }
    ...
    parseObdResponse(response) {
        ...
        if (hexData.startsWith('41')) {
            ...
            if (value !== null) {
                ...
                window.dispatchEvent(new CustomEvent('obd_data', {
    ...
    dispatchStateChange(connected) {
        window.dispatchEvent(new CustomEvent('obd_status', {
...
window.ReferralManager = {
    ...
    captureReferralCode: function() {
        ...
        if (refCode) {
            ...
            if (!existingRef) {
                ...
                if (typeof speak === 'function') {
                    speak("Lien de parrainage détecté. Roulez pour débloquer vos premières récompenses !");
                }
            }
        }
    },
    ...
    shareReferralLink: async function() {
        ...
        const shareText = `Rejoins mon Crew sur l'app ultime pour pilotes de 50cc ! Utilise mon code ${myRefCode} et on gagne des cryptos BVC ! 🏍️🚀`;
    ...
    checkReferralReward: async function(totalKm) {
        ...
        let currentMilestone = parseInt(localStorage.getItem('referralMilestone') || '0', 10);

        // Palier 1 : 20 km (Bienvenue)
        if (totalKm >= 20 && currentMilestone < 1) {
            await this.processMilestoneReward(1, 20, 50, 20, "Félicitations, vous avez dépassé les 20 kilomètres. Votre parrain reçoit 50 BVC, et vous gagnez 20 BVC !");
        }
        // Palier 2 : 100 km (Motard Fidèle)
        else if (totalKm >= 100 && currentMilestone < 2) {
            await this.processMilestoneReward(2, 100, 100, 50, "Incroyable, 100 kilomètres atteints ! Vous êtes maintenant un Motard Fidèle. 50 BVC débloqués.");
        }
        // Palier 3 : 500 km (Pilote Confirmé)
        else if (totalKm >= 500 && currentMilestone < 3) {
            await this.processMilestoneReward(3, 500, 300, 200, "Palier ultime des 500 kilomètres atteint ! Félicitations Pilote Confirmé, un bonus massif vous a été versé.");
        }
    },

    // Méthode générique pour payer les paliers
    processMilestoneReward: async function(milestoneId, kmLimit, referrerReward, refereeReward, voiceMessage) {
        console.log(`mon50cc Referral : Palier ${milestoneId} (${kmLimit}km) atteint !`);
    ...
    checkSafeDrivingBonus: async function(isSafeRide) {
        ...
        console.log("mon50cc Referral : Conduite sécurisée détectée, bonus passif pour le parrain !");
        ...
        if (typeof db !== 'undefined' && window.session) {
            try {
                ...
                if (typeof speak === 'function') {
                     speak("Trajet parfait. Votre conduite prudente a rapporté un bonus à votre parrain !");
                }
            } catch (err) {
...
window.DrivingScore = {
    ...
    init: function() {
        console.log("🏆 DrivingScore Engine: [ READY ]");
        this.currentScore = parseInt(localStorage.getItem('drivingScore') || '100', 10);
        this.createScoreHUD();
    ...
    createScoreHUD: function() {
        ...
        Object.assign(badge.style, {
            position: 'fixed', bottom: '120px', right: '15px', zIndex: '9998',
    ...
    analyzeMotion: function(event) {
        ...
        const avgG = this.smoothAccelHistory.reduce((a, b) => a + b, 0) / this.smoothAccelHistory.length;

        // Freinage brusque
        if (avgG > this.THRESHOLDS.HARD_BRAKE && acc.z < -15) {
            this.applyPenalty('HARD_BRAKE', `Freinage brusque détecté (${avgG.toFixed(1)}G)`);
        }
        // Accélération violente
        else if (avgG > this.THRESHOLDS.HARD_ACCEL && acc.z > 15) {
            this.applyPenalty('HARD_ACCEL', `Accélération agressive (${avgG.toFixed(1)}G)`);
        }
        // Virage agressif (G latéral)
        else if (Math.abs(acc.x) / 9.81 > this.THRESHOLDS.CORNERING) {
            this.applyPenalty('CORNERING', `Virage agressif (${(Math.abs(acc.x) / 9.81).toFixed(1)}G latéral)`);
        }
    },
    ...
    checkSpeed: function(currentSpeedKmh) {
        if (currentSpeedKmh > this.THRESHOLDS.OVER_SPEED) {
            this.applyPenalty('OVER_SPEED', `Excès de vitesse : ${currentSpeedKmh.toFixed(0)} km/h`);
        }
    },

    // Bonus par kilomètre sans infraction (appelé par la télémétrie)
    awardSmoothKm: function() {
        const lastPenaltyTime = this.sessionPenalties.length > 0
        ...
        if (Date.now() - lastPenaltyTime > 300000) {
            this.currentScore = Math.min(100, this.currentScore + this.THRESHOLDS.BONUS_SMOOTH_KM);
            this.saveAndUpdate();
            console.log("🏆 DrivingScore : +1 point (conduite exemplaire)");
        }
    },
    ...
    applyPenalty: function(type, description) {
        const now = Date.now();
        const lastSameType = this.sessionPenalties.filter(p => p.type === type).pop();
        ...
        console.warn(`🏆 DrivingScore : ${penalty} pts → ${description}`);
    ...
    start: function() {
        ...
        console.log("🏆 DrivingScore : Tracking activé");
    },

    stop: function() {
        this.isTracking = false;
        console.log(`🏆 DrivingScore : Session terminée → Score final : ${this.currentScore}/100`);
    },
    ...
    showDetailPanel: function() {
        ...
        Object.assign(panel.style, {
            position: 'fixed', bottom: '200px', right: '15px', width: '280px',
            background: 'rgba(0,0,0,0.92)', border: '1px solid rgba(0,210,255,0.3)',
            borderRadius: '16px', padding: '16px', zIndex: '9999',
            color: '#fff', fontFamily: "'Courier New', monospace', fontSize: '12px",
            backdropFilter: 'blur(10px)', boxShadow: '0 0 30px rgba(0,210,255,0.2)'
        });

        let penaltiesHTML = this.sessionPenalties.length === 0
            ? '<div style="color:#00ff88; text-align:center;">✅ Aucune infraction</div>'
            : this.sessionPenalties.slice(-5).map(p =>
    ...
    generateInsuranceReport: function() {
        return {
            ...
            hardBrakes: this.sessionPenalties.filter(p => p.type === 'HARD_BRAKE').length,
            hardAccels: this.sessionPenalties.filter(p => p.type === 'HARD_ACCEL').length,
            overSpeeds: this.sessionPenalties.filter(p => p.type === 'OVER_SPEED').length,
            aggressiveTurns: this.sessionPenalties.filter(p => p.type === 'CORNERING').length,
            riskLevel: this.currentScore >= 80 ? 'LOW' : this.currentScore >= 50 ? 'MEDIUM' : 'HIGH',
            timestamp: new Date().toISOString()
        };
    }
};
...
window.AccidentReport = {
    init: function() {
        console.log("📄 AccidentReport Engine: [ READY ]");
    ...
    generateReport: function(crashData = null) {
        if (!crashData) {
            ...
            crashData = {
                timestamp: Date.now(),
                location: window.appMap?.currentPos || { lat: 45.367, lng: 4.200 },
                speedAtImpact: 42.5,
                gForce: 6.2,
        ...
        console.log("📄 Génération du rapport d'accident (PDF)...", crashData);
    ...
    showReportUI: function(data) {
        ...
        Object.assign(modal.style, {
            ...
            background: 'rgba(0,0,0,0.95)', zIndex: '10005', overflowY: 'auto',
    ...
    exportPDF: function() {
        ...
        if (typeof speak === 'function') {
            speak("Génération du rapport PDF en cours. Il sera envoyé automatiquement à votre assurance.");
        }
...
window.WeatherAlert = {
    ...
    CONDITIONS: {
        ...
        'RAIN':  { label: "Pluie", danger: true, msg: "Alerte météo. Pluie détectée sur votre itinéraire. Adhérence réduite, ralentissez." },
        'WIND':  { label: "Vent Fort", danger: true, msg: "Alerte météo. Fortes rafales de vent. Maintenez fermement votre guidon." },
        'ICE':   { label: "Risque de Verglas", danger: true, msg: "Alerte critique. Température proche de zéro. Risque extrême de verglas." }
    },

    init: function() {
        console.log("🌦️ WeatherAlert Engine: [ READY ]");
    },
    ...
    start: function() {
        ...
        console.log("🌦️ WeatherAlert : Monitoring activé");
    ...
    stop: function() {
        this.isMonitoring = false;
        if (this.checkInterval) clearInterval(this.checkInterval);
        console.log("🌦️ WeatherAlert : Monitoring désactivé");
    },
    ...
    checkWeather: async function(position) {
        ...
        console.log(`🌦️ WeatherAlert : Analyse météo pour [${position.lat.toFixed(2)}, ${position.lng.toFixed(2)}]...`);
        ...
        if (simulatedWeather !== 'CLEAR' && simulatedWeather !== this.currentCondition) {
    ...
    triggerAlert: function(conditionKey) {
        ...
        console.warn(`🌦️ ALERTE MÉTÉO : ${condition.label}`);
    ...
    showToast: function(condition) {
        ...
        Object.assign(toast.style, {
            ...
            background: 'linear-gradient(135deg, rgba(0,40,80,0.95), rgba(0,0,0,0.95))',
            border: '2px solid #00d2ff', borderRadius: '30px', padding: '12px 24px',
            zIndex: '10001', color: '#fff', fontFamily: "'Inter', sans-serif", fontSize: '14px',
            display: 'flex', alignItems: 'center', gap: '12px',
            boxShadow: '0 0 20px rgba(0,210,255,0.4)', animation: 'slideDown 0.4s ease-out'
        });
...
window.VirtualGarage = {
    ...
    init: function() {
        this.loadData();
        console.log("[VirtualGarage] Initialisé. Véhicule : " + this.data.model);
    },
    ...
    resetPart: function(partKey) {
        ...
        this.saveData();
        this.renderUI();
        if(typeof speak === 'function') speak(`Entretien enregistré pour : ${this.data.parts[partKey].name}.`);
    },
...
window.SafeRider = {
    ...
    init: function() {
        ...
        console.log("[SafeRider] Challenges initialisés.");
    },
    ...
    checkMilestones: function() {
        ...
        let claimed = JSON.parse(localStorage.getItem('safe_rider_claimed') || '[]');
    ...
    awardMilestone: function(milestone) {
        ...
        // Notification UI
        const msg = `🏆 Challenge Réussi : ${milestone.name} ! Vous avez gagné ${milestone.reward} Pts BVC !`;
...
window.ConvoyManager = {
    ...
    createConvoy: async function() {
        ...
        if (!window.session || window.session.isGuest) { alert("Vous devez être connecté pour créer un convoi."); return; }
        ...
        try {
            ...
            if (typeof speak === 'function') speak(`Convoi créé ! Le code est : ${code.split('').join(', ')}.`);
        } catch (e) {
    ...
    joinConvoy: async function(code) {
        if (!window.db) { alert("Connexion Firestore requise."); return; }
        if (!window.session || window.session.isGuest) { alert("Vous devez être connecté."); return; }
        if (!code || code.length !== 4) { alert("Code convoi invalide (4 caractères)."); return; }
        ...
        try {
            ...
            if (typeof speak === 'function') speak(`Vous avez rejoint le convoi ${code}.`);
        } catch (e) {
    ...
    leaveConvoy: async function() {
        ...
        try {
            ...
            await window.db.collection("convoys").doc(this.convoyId)
                .collection("positions").doc(window.session.username).delete();
    ...
    startPositionSharing: function() {
        ...
        const sharePosition = async () => {
            ...
            navigator.geolocation.getCurrentPosition(async (pos) => {
                try {
                    await window.db.collection("convoys").doc(this.convoyId)
                        .collection("positions").doc(window.session.username).set({
                            lat: pos.coords.latitude,
                            lng: pos.coords.longitude,
    ...
    listenToConvoy: function() {
        ...
        this.firestoreUnsubscribe = window.db.collection("convoys").doc(this.convoyId)
            .collection("positions").onSnapshot((snapshot) => {
                this.membersList = [];
                snapshot.forEach((doc) => {
                    const data = doc.data();
    ...
    renderConvoyMarkers: function() {
        ...
        this.membersList.forEach(member => {
            ...
            const marker = L.marker([member.lat, member.lng], { icon: icon }).addTo(window.map);
    ...
    renderUI: function() {
        ...
        if (this.convoyId) {
            ...
            this.membersList.forEach(m => {
                const speed = m.speed ? `${Math.round(m.speed * 3.6)} km/h` : 'En attente GPS';
                membersHTML += `
...
document.addEventListener('DOMContentLoaded', () => {
    console.log("[ConvoyManager] Module chargé.");
});
...
window.LegalDatabase = {
    ...
    france: {
        ...
        casque: {
            ...
            content: "Le port du casque homologué <strong>ECE 22.06</strong> est obligatoire pour tout conducteur et passager de 2-roues motorisé.<br><strong>Sanction :</strong> 135€ d'amende (contravention 4ème classe) + retrait de 3 points.",
        ...
        debridage: {
            ...
            content: "Le débridage d'un cyclomoteur est un <strong>délit</strong>. Vous risquez <strong>135€ d'amende</strong> pour le propriétaire, mais surtout, <strong>votre assurance s'annule</strong> en cas d'accident corporel. Les assureurs se retournent contre vous pour payer les dommages aux victimes.",
        ...
        stupefiants: {
            ...
            content: "Même avec un BSR, vous risquez jusqu'à <strong>4500€ d'amende</strong>, 2 ans de prison, et l'immobilisation du scooter. Il n'y a pas de perte de points sur un BSR. S'il s'agit d'une première infraction, le juge peut faire preuve de clémence si vous montrez des preuves médicales.",
        ...
        alcool: {
            ...
            content: "Pour un permis probatoire ou BSR, la limite légale est de <strong>0,2 g/L</strong>. Vous risquez l'immobilisation immédiate du cyclomoteur et de fortes amendes. Au-delà de 0,8 g/L : délit pénal (2 ans de prison, 4500€).",
        ...
        assurance: {
            ...
            content: "Conduire sans assurance coûte jusqu'à <strong>3750€ d'amende</strong>. En cas d'accident, le Fonds de Garantie indemnise la victime mais <strong>vous réclamera le remboursement</strong>, potentiellement toute votre vie.",
        ...
        fuite: {
            ...
            content: "Cumuler ces délits entraîne des peines de <strong>prison fermes</strong>, des amendes colossales et une interdiction de passer le permis. Ne fuyez jamais un contrôle de police.",
        ...
        stationnement: {
            ...
            content: "Sur un <strong>trottoir</strong> : toléré si le passage piéton (>1,50m) n'est pas entravé. Sur <strong>passage piéton/piste cyclable</strong> : 135€ + fourrière immédiate. Sur <strong>place auto</strong> : toléré si vous payez le stationnement.",
            source: "Légifrance — Art. R417-10 et R417-11",
            url: "legifrance.gouv.fr"
        },
        rgpd: {
            keywords: ["rgpd", "gdpr", "données personnelles", "cnil", "vie privée"],
    ...
    eu: {
        ...
        rgpd: {
            ...
            title: "🇪🇺 RGPD — Règlement (UE) 2016/679",
            content: "Le Règlement Général sur la Protection des Données est le texte de référence en matière de protection des données personnelles dans l'UE. Entrée en vigueur : <strong>25 mai 2018</strong>.<br>Amende max : <strong>20M€ ou 4% du CA mondial</strong>.",
        ...
        dsa: {
            ...
            content: "Le Digital Services Act impose des obligations de <strong>modération</strong> et de <strong>transparence</strong> aux plateformes numériques. Obligation de point de contact, mécanisme de signalement (Art.16), et motivation des décisions de modération (Art.17).",
        ...
        casque_eu: {
            ...
            content: "Depuis juin 2024, seuls les casques homologués <strong>ECE 22.06</strong> peuvent être vendus dans l'UE. Les anciens ECE 22.05 restent utilisables mais ne sont plus fabriqués.",
    ...
    indonesia: {
        ...
        casque: {
            ...
            content: "Le port du casque homologué <strong>SNI</strong> (Standar Nasional Indonesia) est obligatoire pour le conducteur et le passager (Art. 57§2).<br><strong>Sanction :</strong> Jusqu'à 1 mois de prison ou <strong>Rp 250.000</strong> d'amende (Art. 291§1).",
        ...
        code_route: {
            ...
            content: "La loi sur la Circulation et les Transports Routiers régit l'ensemble du trafic en Indonésie. Obligations pour les 2-roues :<br>• Casque SNI obligatoire (Art.106§8)<br>• Rétroviseurs, feux, klaxon, compteur (Art.285§1)<br>• SIM C obligatoire (Art.77)<br>• STNK à jour (Perpol 7/2021)",
        ...
        stnk: {
            ...
            content: "Le STNK est le certificat d'immatriculation obligatoire. Si le STNK expire et n'est pas renouvelé sous <strong>2 ans</strong>, les données du véhicule sont radiées.<br><strong>Opsen Pajak (2025) :</strong> Taxe additionnelle sur le PKB et BBN-KB (UU 1/2022).<br>Depuis 2026, le NIK (KTP) est intégré au SIM.",
        ...
        pdp: {
            ...
            content: "En vigueur depuis le <strong>17 octobre 2024</strong>. Portée extraterritoriale.<br><strong>Sanctions admin. (Art.57) :</strong> Jusqu'à <strong>2% du CA annuel</strong>.<br><strong>Sanctions pénales :</strong> 4-6 ans de prison + Rp 4-6 milliards.<br><strong>Korporasi :</strong> Amende ×10 + gel/dissolution.",
        ...
        contrat: {
            ...
            content: "Régi par le <strong>Code civil indonésien</strong> (KUH Perdata), hérité du droit romano-hollandais. L'Indonésie n'a <strong>pas ratifié</strong> la Convention de Vienne (CISG).",
        ...
        hierarchie: {
            ...
            content: "Système mixte (adat / romano-hollandais / national / musulman à Aceh).<br>1. <strong>UUD 1945</strong> — Constitution<br>2. <strong>UU</strong> — Lois du Parlement<br>3. <strong>PP</strong> — Règlements gouvernementaux<br>4. <strong>Perpres</strong> — Décrets présidentiels<br>5. <strong>Perda</strong> — Règlements régionaux",
        ...
        immobilier: {
            ...
            content: "Les étrangers ne peuvent posséder de terres directement (<strong>Hak Milik</strong>), mais peuvent acquérir des droits d'usage (<strong>Hak Pakai</strong>) ou investir via des sociétés (<strong>PT PMA</strong>).",
            source: "JDIH — peraturan.bpk.go.id",
            url: "peraturan.bpk.go.id"
        },
        travail: {
            ...
            content: "Loi n°13/2003 = texte principal. Modifiée par la <strong>loi omnibus n°11/2020</strong> (Cipta Kerja) pour faciliter l'investissement (contrats, licenciements, heures supplémentaires).",
            source: "JDIH — jdih.kemenkumham.go.id",
            url: "jdih.kemenkumham.go.id"
        },
        langue: {
            ...
            content: "L'indonésien (<em>Bahasa Indonesia</em>) est la langue officielle de l'éducation et des médias. Les langues régionales et étrangères sont autorisées sous conditions.",
    ...
    usa: {
        ...
        assurance: {
            ...
            content: "L'assurance moto est obligatoire dans <strong>48 des 50 États</strong> (sauf Floride et Montana pour la responsabilité civile). Les minimums de couverture varient considérablement par État. En Californie : 15/30/5 (en milliers de $).",
        ...
        ccpa: {
            ...
            content: "Le <strong>CCPA</strong> (California Consumer Privacy Act, 2020) et son amendement <strong>CPRA</strong> offrent aux résidents californiens des droits proches du RGPD : droit de savoir, de suppression, de refus de vente. <strong>Amende :</strong> $2.500/violation, $7.500/violation intentionnelle.",
        ...
        coppa: {
            ...
            content: "La <strong>Children's Online Privacy Protection Act</strong> interdit la collecte de données personnelles d'enfants de moins de 13 ans sans consentement parental vérifiable. <strong>Amende :</strong> jusqu'à $50.120/violation (FTC).",
            source: "FTC — ftc.gov",
            url: "ftc.gov"
        }
    },
    ...
    uk: {
        ...
        _keywords: ["royaume-uni", "uk", "angleterre", "british", "anglais", "london"],

        casque: {
            keywords: ["casque", "helmet"],
            title: "🇬🇧 Casque Moto — Road Traffic Act 1988 §16",
            content: "Le port du casque homologué <strong>BS 6658:1985</strong> ou <strong>UNECE R22.05/22.06</strong> est obligatoire. Les Sikhs portant un turban sont exemptés (§16§2).<br><strong>Sanction :</strong> Fixed Penalty Notice de <strong>£100</strong>.",
        ...
        permis: {
            ...
            content: "Formation obligatoire : <strong>CBT</strong> (Compulsory Basic Training). Catégories :<br>• <strong>AM</strong> : Cyclomoteur ≤ 50cc<br>• <strong>A1</strong> : ≤ 125cc (16+)<br>• <strong>A2</strong> : ≤ 35kW (19+)<br>• <strong>A</strong> : Illimité (24+ ou 21+ avec 2 ans d'A2)",
        ...
        uk_gdpr: {
            ...
            content: "Post-Brexit, le Royaume-Uni a conservé les principes du RGPD via le <strong>UK GDPR</strong> et le <strong>Data Protection Act 2018</strong>. L'autorité de contrôle est l'<strong>ICO</strong> (Information Commissioner's Office). Amende max : <strong>£17.5M ou 4% du CA</strong>.",
    ...
    japan: {
        ...
        casque: {
            ...
            content: "Le port du casque homologué <strong>PSC/SG</strong> est obligatoire pour tous les conducteurs et passagers de 2-roues. Les casques doivent porter le marquage <strong>PSCマーク</strong>.<br>Norme : <strong>JIS T 8133</strong>.",
            source: "Japanese Law Translation — Road Traffic Act (道路交通法)",
            url: "japaneselawtranslation.go.jp"
        },
        permis: {
            keywords: ["permis", "licence", "conduire"],
            title: "🇯🇵 Permis Moto (免許) — Road Traffic Act",
            content: "Catégories :<br>• <strong>原付</strong> (Gentsuki) : ≤ 50cc (16+)<br>• <strong>小型</strong> : ≤ 125cc<br>• <strong>普通</strong> : ≤ 400cc<br>• <strong>大型</strong> : Illimité (18+)<br>Examen pratique obligatoire en circuit fermé.",
            source: "Japanese Law Translation — 道路交通法",
            url: "japaneselawtranslation.go.jp"
        },
        appi: {
            keywords: ["appi", "donnée", "data", "ppc", "privacy"],
            title: "🇯🇵 APPI — Act on Protection of Personal Information",
            content: "Révisée en 2022. L'APPI est supervisée par la <strong>PPC</strong> (Personal Information Protection Commission). Le Japon bénéficie d'une <strong>décision d'adéquation</strong> avec l'UE (RGPD). Transferts transfrontaliers strictement encadrés.",
    ...
    china: {
        ...
        casque: {
            ...
            content: "Depuis la campagne nationale de 2020, le port du casque est obligatoire pour les conducteurs et passagers de 2-roues dans toute la Chine. Norme obligatoire : <strong>GB 811-2022</strong> (mise à jour de la norme nationale).",
            source: "NPC — Road Traffic Safety Law (道路交通安全法)",
            url: "npc.gov.cn"
        },
        pipl: {
            keywords: ["pipl", "donnée", "data", "privacy"],
            title: "🇨🇳 PIPL — Personal Information Protection Law (2021)",
            content: "En vigueur depuis le <strong>1er novembre 2021</strong>. Portée extraterritoriale. Consentement séparé requis pour les données sensibles. <strong>Amende :</strong> jusqu'à <strong>50M RMB ou 5% du CA annuel</strong>. Transferts transfrontaliers soumis à évaluation de sécurité obligatoire (CAC).",
            source: "NPC — 个人信息保护法",
            url: "npc.gov.cn"
        },
        dsl: {
            ...
            content: "La Loi sur la Sécurité des Données (DSL) classe les données par niveau d'importance (national, important, général). Les données « importantes » et « nationales » exigent des évaluations de risque et des stockages localisés.",
            source: "NPC — 数据安全法",
            url: "npc.gov.cn"
        }
    },
    ...
    india: {
        ...
        casque: {
            ...
            content: "Le port du casque homologué <strong>ISI (BIS)</strong> est obligatoire pour le conducteur et le passager. Norme : <strong>IS 4151:2015</strong>.<br><strong>Sanction :</strong> ₹1.000 d'amende + suspension du permis (3 mois).<br>Exception : Les Sikhs portant un turban sont exemptés dans certains États.",
        ...
        permis: {
            ...
            content: "Deux catégories :<br>• <strong>MCWG</strong> (Motor Cycle With Gear) : Moto avec vitesses<br>• <strong>MCWOG</strong> : Scooter sans vitesses<br>Âge minimum : <strong>18 ans</strong> (16 ans pour les ≤50cc dans certains États).",
        ...
        dpdp: {
            ...
            content: "En vigueur depuis <strong>2023</strong>. Droits des « Data Principals » : consentement, rectification, effacement. Possibilité de nommer un représentant légal. <strong>Amende :</strong> jusqu'à <strong>₹250 crore</strong> (≈ 27M€). Supervision par le Data Protection Board of India.",
    ...
    brazil: {
        ...
        casque: {
            ...
            content: "Le port du casque homologué <strong>INMETRO</strong> est obligatoire pour le conducteur et le passager de moto. Le viseur est aussi obligatoire.<br><strong>Sanction :</strong> Infraction grave — <strong>R$293,47</strong> + 7 points sur le CNH + rétention du véhicule.",
        ...
        cnh: {
            ...
            content: "Catégorie <strong>A</strong> obligatoire pour les 2-roues. Âge minimum : <strong>18 ans</strong>. Formation obligatoire incluant cours théoriques (45h) et pratiques (20h). Système de points : <strong>40 pts/an = suspension</strong>.",
        ...
        lgpd: {
            ...
            content: "La LGPD est le « RGPD brésilien ». En vigueur depuis <strong>septembre 2020</strong>. Supervisée par l'<strong>ANPD</strong> (Autoridade Nacional de Proteção de Dados). <strong>Amende :</strong> jusqu'à <strong>2% du CA au Brésil</strong>, plafonnée à R$50M par infraction.",
    ...
    singapore: {
        ...
        casque: {
            ...
            content: "Le casque homologué <strong>PSB/Spring SG</strong> (ou UN R22) est obligatoire. <br><strong>Sanction :</strong> Amende jusqu'à <strong>S$1.000</strong> et/ou 3 mois de prison.",
        ...
        pdpa: {
            ...
            content: "Supervisée par la <strong>PDPC</strong>. Consentement éclairé obligatoire. Droit d'accès et de correction rapide.<br><strong>Amende :</strong> jusqu'à <strong>S$1M ou 10% du CA annuel</strong> (depuis la révision 2020).",
    ...
    south_africa: {
        ...
        casque: {
            ...
            content: "Le port du casque homologué <strong>SABS (SANS 55)</strong> est obligatoire pour tous les conducteurs et passagers de 2-roues.<br><strong>Sanction :</strong> Amende et points de démérite.",
        ...
        popia: {
            ...
            content: "En vigueur depuis <strong>juillet 2021</strong>. L'<strong>Information Regulator</strong> est l'autorité de contrôle. Traitement licite et raisonnable obligatoire. Droit d'accès, de correction, et de suppression.<br><strong>Amende :</strong> jusqu'à <strong>R10M</strong> et/ou 10 ans de prison.",
    ...
    nigeria: {
        ...
        casque: {
            ...
            content: "Le port du casque est obligatoire pour les conducteurs et passagers de motos (<em>Okada</em>). Réglementation appliquée par le <strong>FRSC</strong> (Federal Road Safety Corps).<br><strong>Sanction :</strong> ₦2.000 d'amende.",
        ...
        ndpr: {
            ...
            content: "Remplace le NDPR de 2019. Crée la <strong>NDPC</strong> (Nigeria Data Protection Commission) comme autorité indépendante. Consentement obligatoire. Notifications de violation sous <strong>72h</strong>.<br><strong>Amende :</strong> jusqu'à <strong>2% du CA mondial</strong> ou ₦10M.",
    ...
    morocco: {
        ...
        casque: {
            ...
            content: "Le port du casque homologué est obligatoire pour les conducteurs et passagers de 2-roues motorisés.<br><strong>Sanction :</strong> Amende de <strong>400 à 700 DH</strong>, immobilisation du véhicule, et retrait de permis possible.",
        ...
        permis: {
            ...
            content: "Catégories :<br>• <strong>A1</strong> : Cyclomoteur ≤ 50cc (16+)<br>• <strong>A</strong> : Toute moto (18+)<br>Système de permis à points depuis 2010.",
        ...
        loi_0908: {
            ...
            content: "En vigueur depuis <strong>2009</strong>. Supervisée par la <strong>CNDP</strong> (Commission Nationale de Contrôle de la Protection des Données). Inspirée du modèle français (CNIL). Droits d'accès, de rectification et d'opposition.",
    ...
    // ═══════════════════════════════════════════════════════════════
    // 🇹🇭 THAÏLANDE — Source : Royal Thai Police
    // ═══════════════════════════════════════════════════════════════
    thailand: {
        _flag: "🇹🇭",
        _name: "Thaïlande",
        ...
        casque: {
            ...
            content: "Le port du casque est obligatoire pour les conducteurs et passagers de moto. Norme : <strong>TIS 369</strong> (Thai Industrial Standard).<br><strong>Sanction :</strong> Amende de <strong>500 THB</strong>.",
        ...
        pdpa_th: {
            ...
            content: "En vigueur depuis <strong>juin 2022</strong>. Très inspirée du RGPD. Consentement explicite requis pour les données sensibles. <strong>Amende :</strong> jusqu'à <strong>5M THB</strong> + sanctions pénales (1 an de prison et/ou 1M THB).",
    ...
    australia: {
        ...
        casque: {
            ...
            content: "Le port du casque homologué <strong>AS/NZS 1698:2006</strong> (ou UNECE R22) est obligatoire dans tous les États et Territoires.<br><strong>Sanction :</strong> Varie par État. Ex NSW : <strong>A$349</strong> + 3 points de démérite.",
        ...
        privacy_act: {
            ...
            content: "Supervisée par l'<strong>OAIC</strong> (Office of the Australian Information Commissioner). Les 13 <strong>Australian Privacy Principles (APPs)</strong> régissent la collecte, l'utilisation et la sécurité des données.<br><strong>Amende :</strong> jusqu'à <strong>A$50M</strong>, 3× le bénéfice obtenu, ou 30% du CA (le plus élevé).",
    ...
    canada: {
        ...
        casque: {
            ...
            content: "Le casque est obligatoire dans <strong>toutes les provinces</strong>. Normes acceptées : <strong>DOT (FMVSS 218)</strong>, <strong>Snell</strong>, <strong>ECE 22.05/22.06</strong>.<br><strong>Sanction :</strong> Varie par province. Ontario : <strong>C$110</strong>.",
        ...
        pipeda: {
            ...
            content: "Loi fédérale sur la protection des renseignements personnels dans le secteur privé. Supervisée par le <strong>Commissariat à la protection de la vie privée</strong>. Remplacée progressivement au Québec par la <strong>Loi 25</strong> (2023).<br><strong>Amende :</strong> jusqu'à <strong>C$100.000</strong> (PIPEDA), C$25M ou 4% du CA (Loi 25 QC).",
...
/**
 * 🔍 Moteur de recherche dans la base juridique mondiale
...
window.LegalDatabase.search = function(query) {
    ...
    if (targetCountries.length === 0) {
        targetCountries = Object.keys(this).filter(k => typeof this[k] === 'object' && k !== 'search');
    }
    ...
    for (const countryKey of targetCountries) {
        ...
        for (const [topicKey, topic] of Object.entries(country)) {
            if (topicKey.startsWith('_') || typeof topic !== 'object' || !topic.keywords) continue;
