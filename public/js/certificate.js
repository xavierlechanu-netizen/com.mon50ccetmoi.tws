...
window.Certificate = {
    ...
    generateBattery: function(tier = 'premium', predefinedCertId = null) {
        if (typeof speak === 'function') speak("Génération du certificat de batterie pour la revente.");
        ...
        overlay.style = "position:fixed; top:0; left:0; width:100%; height:100%; z-index:40000; background:#fff; color:#000; padding:40px; font-family:'Inter', sans-serif; overflow-y:auto;";
                ...
                <div style="position: relative; z-index: 1;">
                    ...
                    <div style="display: flex; gap: 20px; margin-bottom: 30px;">
                        ...
                        <div style="flex: 2; border: 1px solid #ddd; border-radius: 8px; padding: 20px;">
                            ...
                            <p style="margin: 5px 0; display:flex; justify-content:space-between;"><span>Âge batterie :</span> <strong>${bState.age}</strong></p>
                        </div>
                    </div>
    ...
    purchaseBatteryCert: async function(tier) {
        ...
        if (typeof speak === 'function') speak("Connexion bancaire sécurisée pour l'achat du certificat.");
        
        const loader = document.createElement('div');
        loader.id = "payment-loader";
        loader.style = "position:fixed; top:0; left:0; width:100%; height:100%; z-index:50000; background:rgba(0,0,0,0.95); color:#fff; display:flex; flex-direction:column; justify-content:center; align-items:center; font-family:'Inter', sans-serif;";
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error("Erreur serveur lors de la création de l'ordre Revolut.");
            }
            ...
            instance.payWithPopup({
                onSuccess: async () => {
                    if (typeof speak === 'function') speak("Paiement Revolut validé. Génération de votre document.");
                    
                    // Sauvegarde Firestore uniquement si le paiement a réussi
                    try {
                        if (window.firebase && window.firebase.firestore && window.session && window.session.uid) {
                            const bState = window.batteryState || {
                                soh: "92%", mileage: "12 450 km", cycles: "345 / 1000",
                                age: "2 ans et 3 mois", style: "Éco-Responsable / Fluide", date: "15 Janvier 2024"
                            };
                            await window.firebase.firestore().collection("battery_certificates").doc(certId).set({
                            ...
                            console.log("[Firestore] Certificat de batterie sauvegardé :", certId);
                        } else {
                            console.warn("[Firestore] Mode hors-ligne. Le certificat n'est pas sauvegardé.");
                        }
                    } catch (err) {
                ...
                onCancel: () => {
                    console.log("Paiement Revolut annulé par l'utilisateur.");
    ...
    submitManualData: function() {
        ...
        const ageStr = ageYears > 0 ? `${ageYears} ans et ${ageMonths} mois` : `${ageMonths} mois`;
        ...
        if(elSoh) {
            ...
            elSoh.style.color = soh > 80 ? '#00ffcc' : (soh > 50 ? '#ffb703' : '#ff3366');
        ...
        if(elStyle) {
            ...
            elStyle.style.color = style === "Sportive / Nerveuse" ? "#ff3366" : "#ffb703";
        }

        // Fermer la fenêtre modale
        document.getElementById('battery-manual-screen').classList.add('hidden');
        if (typeof speak === 'function') speak("Données enregistrées. État de santé estimé à " + soh + " pourcent.");
    }
};
